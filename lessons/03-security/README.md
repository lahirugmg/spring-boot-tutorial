...
> This lesson owns the **`security` schema** in `appdb`; lesson 02 owns `public`. See
> [application.yml](src/main/resources/application.yml) for why that separation exists.

---

## Theory: authentication, authorization, and stateless JWTs

**Authentication vs. authorization.** Authentication answers "who are you?" — verifying an
identity, usually via credentials. Authorization answers "what are you allowed to do?" —
deciding whether that already-verified identity can perform a specific action. These are
genuinely separate concerns with separate failure codes: a failed authentication is
**401 Unauthorized** ("I don't know who you are"); a failed authorization is
**403 Forbidden** ("I know exactly who you are, and the answer is no").

**The filter chain.** Spring Security is, mechanically, a chain of servlet `Filter`s registered
in front of your controllers — one for extracting/validating a token, one for building the
authentication object, one for enforcing URL-level access rules, and so on, each free to
short-circuit the request. Order matters enormously: a filter that's never reached never runs.
That's why "walk me through the filter chain" is a genuine interview staple rather than trivia —
understanding it is understanding how a request actually gets accepted or rejected.

**Stateful sessions vs. stateless tokens.** The traditional model authenticates once, stores the
identity server-side (an `HttpSession`), and hands the client a session-id cookie to prove who
they are on later requests. That doesn't scale horizontally without sticky sessions or a shared
session store. The stateless alternative: the server issues a signed token (a JWT) containing
the identity and claims; every later request carries that token, and the server verifies the
signature instead of doing a session lookup. No server-side state, easy horizontal scaling — at
the cost of not being able to instantly revoke a token before it expires.

**JWT anatomy.** A JWT is three base64url segments joined by dots: `header.payload.signature`.
The header names the signing algorithm; the payload is a set of claims (`sub`, `exp`, custom
fields like roles) — **plain, readable JSON, not encrypted**, decodable by anyone holding the
token; the signature is the part that actually can't be forged without the signing key. The
practical consequence: never put secrets in a JWT payload, and always verify the signature
server-side — never trust an unverified `alg: none` token.

**Method security hits the same proxy wall.** `@PreAuthorize` on a service method works exactly
like `@Transactional` — Spring wraps the bean in a proxy, and the check runs in that proxy, not
in the target object. Call the annotated method via `this` from another method on the same bean
and the check is skipped entirely. On a transaction that's a bug; on an authorization check it's
a privilege-escalation vulnerability — precisely the "deliberate vulnerability" below.
...
## Two bugs this lesson was built around
...
```bash
./mvnw -f lessons/03-security/pom.xml verify
```
...
Previous: [Lesson 02 — Data & JPA](../02-data-jpa/) · Next: [Lesson 04 — Caching & Resilience](../04-resilience/)
