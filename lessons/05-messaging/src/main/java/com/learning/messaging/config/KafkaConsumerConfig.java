...
public class KafkaConsumerConfig {
    ...
    DefaultErrorHandler kafkaErrorHandler(
            @Qualifier("kafkaTemplate") KafkaOperations<String, Object> kafkaOperations) {
        ...
        // Exponential backoff, bounded by ELAPSED TIME, not by attempt count: keep retrying
        // with 500ms, 1000ms, 2000ms, ... delays until the cumulative wait exceeds
        // maxElapsedTime, then stop and hand off to the recoverer. That is 5 delivery
        // attempts in practice with these numbers, not 3 — measure it yourself with
        // POST /api/orders/poison and watch the "attempt" field on redelivery.
