...
## Theory: JPA, Hibernate, and the persistence context

**JDBC vs. JPA vs. Hibernate vs. Spring Data JPA.** JDBC is the low-level API for talking to a
relational database — you write SQL, map `ResultSet` rows to objects by hand. JPA (Jakarta
Persistence API) is a *specification* for object-relational mapping: annotate a class as an
`@Entity` and something implements the mapping between that object graph and rows. Hibernate is
the implementation doing that work here (and in most Spring Boot apps). Spring Data JPA sits a
layer higher still: it generates repository implementations from interface method names and
`@Query` annotations, so most CRUD and query code is never hand-written.

**The persistence context (first-level cache).** An `EntityManager` tracks every entity it has
loaded or saved during one transaction. Load the same row twice inside that context and the
second load is free — same Java object, no second query. That's why the N+1 demo below reports
7 queries instead of 21: repeated authors get deduplicated *within* one context. It's also why
changes to a managed entity are picked up automatically at flush time ("dirty checking")
without an explicit `save()` call.

**Lazy vs. eager loading, and N+1.** Associations can load immediately with the owning entity
(`EAGER`) or on first access (`LAZY`). The N+1 problem: fetch 20 books with a lazy `@ManyToOne`
author, then naively touch `book.getAuthor().getName()` for each — that's 20 separate
`SELECT`s fired after the original, 21 round trips for what could be one join. It's invisible on
a laptop with 5 rows; it's an outage at production scale. The fixes below — `JOIN FETCH`,
`@EntityGraph`, DTO projections, a global batch-fetch size — trade off control, code, and layer
purity differently, which is exactly what the measurements let you compare.

**Transactions and `@Transactional`.** A transaction groups operations so they commit or roll
back as a unit. Spring's `@Transactional` is AOP: it wraps the annotated method in a proxy that
opens a transaction before the call and commits or rolls back after. Two consequences trip up
almost everyone at least once: it's proxy-based, so **calling the method via `this` from inside
the same class bypasses the proxy — no transaction happens**; and the default rollback rule only
fires for unchecked exceptions (`RuntimeException`/`Error`) — a checked exception commits
whatever ran before it was thrown, unless you say `rollbackFor` explicitly.

**Migrations vs. `ddl-auto`.** Hibernate can generate schema DDL from your entities
(`ddl-auto: update`/`create`) — convenient for a throwaway prototype, dangerous for anything
real, since it has no concept of deployment history and can silently choose a destructive
change. Flyway (used here) takes the opposite approach: versioned, checked-in SQL scripts are
the source of truth, applied in order and tracked in a metadata table; `ddl-auto: validate` then
just confirms the entities match what Flyway actually created — catching drift at startup
instead of at 2am.
...
```bash
./mvnw -f lessons/02-data-jpa/pom.xml verify     # 21 tests against a real Postgres container
```
...
Previous: [Lesson 01 — Core & Web](../01-core-web/) · Next: [Lesson 03 — Security](../03-security/)
